...
{
#if (DSP)
  DSP_Out_Buff_Read (codec_buff.tx, CODEC_BUFF_HALF_SIZE);
#if (!EMBED_ADC)
  DSP_In_Buff_Write (codec_buff.rx, CODEC_BUFF_HALF_SIZE);
#endif /* ADC */
#endif /* DSP */
}
...
{
#if (DSP)
  DSP_Out_Buff_Read (&codec_buff.tx [CODEC_BUFF_HALF_SIZE], CODEC_BUFF_HALF_SIZE);
#if (!EMBED_ADC)
  DSP_In_Buff_Write (&codec_buff.rx [CODEC_BUFF_HALF_SIZE], CODEC_BUFF_HALF_SIZE);
#endif /* ADC */
#endif /* DSP */
}
...
void codec_buff_init (void)
{
  memset (codec_buff.rx, 0, CODEC_BUFF_SIZE * 2U);
  memset (codec_buff.tx, 0, CODEC_BUFF_SIZE * 2U);

  HAL_I2SEx_TransmitReceive_DMA (I2S_IF, codec_buff.tx, codec_buff.rx, CODEC_BUFF_SIZE);
}
...
/* Codec uses BCLK
  //Write 0x22 to Register 102 for PLLCLK_IN uses BCLK
  codec_write_reg (102, 0x22);
  //Write 0x91 to Register 3 for PLLCLK_IN uses BCLK and P = 1
  codec_write_reg (3, 0x91);
  //If PLLCLK_IN uses BCLK then D = 0000, R = 1 and Registers 5, 6, 11 is default
  //If BCLK = 48 kHz write 0x40 to Register 4 to set J = 16
  //If BCLK = 96 kHz write 0x80 to Register 4 to set J = 32
  codec_write_reg (4, 0x80);
*/
  ...
  //codec_write_reg (12, 0x50);  //Set ADC high-pass filter –3dB frequency = 0.0045 × ADC fs

  //codec_set_gain (15, 54);     //Set PGA_L gain = 27dB
  //codec_set_gain (16, 54);     //Set PGA_R gain = 27dB
  ...
  //data = 0x00; // powered down
  //data = 0x40; // powered to 2V
  //data = 0x80; // powered to 2.5V
  //data = 0xC0; // powered to AVDD
  codec_write_reg (25, 0x40);
...
#if (CODEC)
  HAL_GPIO_WritePin (I2S3_RES_GPIO_Port, I2S3_RES_Pin, GPIO_PIN_RESET);
  HAL_Delay (100);
  HAL_GPIO_WritePin (I2S3_RES_GPIO_Port, I2S3_RES_Pin, GPIO_PIN_SET);
